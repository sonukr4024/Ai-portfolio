...
export default function ScrollToBottom({ visible, onClick }: Props) {
  ...
  return (
    <button
      onClick={onClick}
      className="absolute bottom-6 left-1/2 -translate-x-1/2 px-3 py-1.5 rounded-full bg-white border border-[var(--color-border)] hover:border-[var(--color-accent)]/40 flex items-center gap-1.5 text-[11px] text-[var(--color-muted)] hover:text-[var(--color-accent)] transition-all duration-300 shadow-lg cursor-pointer animate-bounce-in hover:shadow-xl hover:scale-105"
